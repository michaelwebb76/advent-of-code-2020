...
part1Solution =
  print $
    length
      ( filter
          ( \string -> case parseString parsePasswordRuleAndPassword mempty string of
              Success (passwordRule, password) ->
                passwordValidOldPolicy passwordRule password
              _ ->
                False
          )
          longListPasswordRulesAndPasswords
      )
  ...
  print $
    length
      ( filter
          ( \string -> case parseString parsePasswordRuleAndPassword mempty string of
              Success (passwordRule, password) ->
                passwordValidNewPolicy passwordRule password
              _ ->
                False
          )
          longListPasswordRulesAndPasswords
...
data PasswordRule = PasswordRule Integer Integer Char deriving (Eq, Show)

parsePasswordRuleAndPassword :: Parser (PasswordRule, Password)
parsePasswordRuleAndPassword =
  (,) <$> (parsePasswordRule <* string ": ") <*> some letter

parsePasswordRule :: Parser PasswordRule
parsePasswordRule =
  PasswordRule <$> (integer <* char '-') <*> integer <*> letter

passwordValidNewPolicy :: PasswordRule -> Password -> Bool
passwordValidNewPolicy (PasswordRule min max char) password =
  let charAtMin = password !! (fromInteger min - 1)
      charAtMax = password !! (fromInteger max - 1)
   in (charAtMin == char || charAtMax == char) && (charAtMin /= charAtMax)

passwordValidOldPolicy :: PasswordRule -> Password -> Bool
passwordValidOldPolicy (PasswordRule min max char) password =
  let charCount = toInteger $ length (filter (char ==) password)
   in charCount >= min && charCount <= max

shortListPasswordRulesAndPasswords :: [String]
shortListPasswordRulesAndPasswords =
  [ "1-3 a: abcde",
    "1-3 b: cdefg",
    "2-9 c: ccccccccc"
  ]

longListPasswordRulesAndPasswords :: [String]
longListPasswordRulesAndPasswords =
