...
part1Reducer inputs =
  maximum $ map (\(Input _ _ _ _ seatId) -> seatId) inputs

part2Reducer :: [Input] -> [SeatId]
part2Reducer inputs =
  let allPossibleSeatIds =
        [0 .. (127 * 8 - 1)]
   in filter
        ( \seatId ->
            not $ any (\(Input _ _ _ _ iSeatId) -> seatId == iSeatId) inputs
        )
        allPossibleSeatIds
...
data FOrB = F | B deriving (Eq, Show)

data LOrR = L | R deriving (Eq, Show)
...
data Input
  = Input [FOrB] [LOrR] Row Column SeatId
  deriving (Eq, Show)
...
parseInput =
  some (convertToInput <$> parseFOrBs <*> (parseLOrRs <* carriageReturnOrEof))

carriageReturnOrEof :: Parser ()
carriageReturnOrEof = choice [void $ string "\n", eof]

parseFOrBs :: Parser [FOrB]
parseFOrBs =
  some (choice [F <$ char 'F', B <$ char 'B'])

parseLOrRs :: Parser [LOrR]
parseLOrRs = some (choice [L <$ char 'L', R <$ char 'R'])

convertToInput :: [FOrB] -> [LOrR] -> Input
convertToInput fOrBs lOrRs =
  let column =
        fst $
          foldr
            ( \lOrR (total, power) -> case lOrR of
                L ->
                  (total, power + 1)
                R ->
                  (total + 2 ^ power, power + 1)
            )
            (0, 0)
            lOrRs
      row =
        fst $
          foldr
            ...
            fOrBs
      seatId = row * 8 + column
   in Input fOrBs lOrRs row column seatId
