...
part1Reducer = length . nub . bagsContaining "shiny gold"

bagsContaining :: String -> [Input] -> [Input]
bagsContaining bagName bags =
  let bagsDirectlyContaining = filter (\(Bag _ subbags) -> any (\(_, Bag name _) -> name == bagName) subbags) bags
   in bagsDirectlyContaining
...
data Bag = Bag String [(Int, Bag)] deriving (Eq, Show)
...
parseInput =
  some
    ( Bag <$> parseBagName
        <* string " contain "
          <*> (catMaybes <$> sepBy parseRule (string ", "))
        <* char '.'
        <* choice [void $ string "\n", eof]
    )

parseRule :: Parser (Maybe (Int, Bag))
parseRule =
  choice
    [ Nothing <$ string "no other bags",
      (\int bagName -> Just (int, Bag bagName []))
        <$> (read <$> some digit <* char ' ') <*> parseBagName
    ]
...
longInput :: String
longInput =
  [r|shiny indigo bags contain 4 vibrant lime bags.
