...
testWithWorkings matcher reducer =
  let parsedInput = parseString parseInput mempty part2Valid
...
data ParsedField = ParsedField Char Char Char String deriving (Eq, Show)

-- Parsers

parseInput :: Parser [Input]
parseInput =
  some (parsedFieldsToInput <$> some parseParsedField <* carriageReturnOrEof)

parsedFieldsToInput :: [ParsedField] -> Input
parsedFieldsToInput parsedFields =
  let findParsedFieldValue fieldName =
        (\(ParsedField _ _ _ value) -> value)
          <$> find (\(ParsedField a b c _) -> fieldName == [a, b, c]) parsedFields
      partialPassportBuilder =
        ValidPassport <$> findParsedFieldValue "byr"
          <*> findParsedFieldValue "iyr"
          <*> findParsedFieldValue "eyr"
          <*> findParsedFieldValue "hgt"
          <*> findParsedFieldValue "hcl"
          <*> findParsedFieldValue "ecl"
          <*> findParsedFieldValue "pid"
   in case partialPassportBuilder of
        Just f ->
          f (findParsedFieldValue "cid")
        Nothing ->
          InvalidPassport

carriageReturnOrEof :: Parser ()
carriageReturnOrEof = choice [void $ string "\n", eof]

parseParsedField :: Parser ParsedField
parseParsedField =
  ParsedField <$> letter <*> letter <*> letter <*> (char ':' *> some (noneOf "\n ") <* optional (oneOf "\n "))
...
part2Matcher input =
  case input of
    InvalidPassport ->
      False
    ValidPassport birthYear issueYear expirationYear height hairColor eyeColor passportID _ ->
...
validBirthYear :: BirthYear -> Bool
validBirthYear birthYear = length birthYear == 4 && birthYear >= "1920" && birthYear <= "2002"

validIssueYear :: IssueYear -> Bool
validIssueYear issueYear = length issueYear == 4 && issueYear >= "2010" && issueYear <= "2020"

validExpirationYear :: ExpirationYear -> Bool
validExpirationYear expirationYear =
  length expirationYear == 4 && expirationYear >= "2020" && expirationYear <= "2030"

validHeight :: Height -> Bool
validHeight height =
  case parseString ((,) <$> integer <*> choice [string "cm", string "in"] <* eof) mempty height of
    Success (number, units) ->
      case units of
        "cm" ->
...
validHairColor :: HairColor -> Bool
validHairColor hairColor =
  case parseString (char '#' *> some hexDigit <* eof) mempty hairColor of
    Success _ ->
      True
    _ ->
      False

validEyeColor :: EyeColor -> Bool
validEyeColor eyeColor =
  case parseString
    ( choice
        [ string "amb",
...
validPassportID :: PassportID -> Bool
validPassportID passportID =
  case parseString (digit <* digit <* digit <* digit <* digit <* digit <* digit <* digit <* digit <* eof) mempty passportID of
    Success _ ->
      True
    _ ->
      False
