...
part1Reducer =
  sum . map (\(Input input) -> uniqueAnswers input)

uniqueAnswers :: [IndividualAnswer] -> Int
uniqueAnswers =
  length . nub . concatMap (\(IndividualAnswer chars) -> chars)

part2Reducer :: [Input] -> Int
part2Reducer =
  sum . map (\(Input input) -> commonAnswers input)

commonAnswers :: [IndividualAnswer] -> Int
commonAnswers individualAnswers =
  let individualAnswersStrings = map (\(IndividualAnswer string) -> string) individualAnswers
   in length $ foldr intersect (head individualAnswersStrings) individualAnswersStrings
...
newtype IndividualAnswer = IndividualAnswer [Char] deriving (Eq, Show)

newtype Input
  = Input [IndividualAnswer]
  deriving (Eq, Show)
...
parseInput =
  some (Input <$> some parseIndividualAnswer <* choice [void $ string "\n", eof])

parseIndividualAnswer :: Parser IndividualAnswer
parseIndividualAnswer = IndividualAnswer <$> some letter <* optional (string "\n")
